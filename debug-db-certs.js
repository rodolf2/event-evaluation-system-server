  ...
  try {
    ...
    templates.forEach(t => {
      ...
      if (t.canvasData?.objects?.length > 0) {
        console.log(`- Sample Object:`, JSON.stringify(t.canvasData.objects[0], null, 2));
      }
    });

    const forms = await Form.find({ isCertificateLinked: true }).sort({ updatedAt: -1 }).limit(5);
    ...
    forms.forEach(f => {
      ...
      if (f.certificateCanvasData) {
        const data = typeof f.certificateCanvasData === 'string' ? JSON.parse(f.certificateCanvasData) : f.certificateCanvasData;
