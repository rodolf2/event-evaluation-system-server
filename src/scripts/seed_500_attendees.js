...
// Target form title
const TARGET_FORM_TITLE = "500 Attendees Mixed Departments Event";
...
const positiveComments = [
  ...
  "Nakabuti talaga sa akin ang attendance. Balik-balikan ko!",
  "Ang ambiance ng venue ay conducive sa learning talaga.",
  "Very comprehensive ang program. Learnings are applicable!",
  "Appreciate ko ang effort ng organizers. Excellent execution!",
  "Ang format ay engaging at hindi boring. Loved it so much!",
  "Professional management ng event from start to finish.",
  "Sobrang worth it ang registration fee para sa quality ng content!",
  "Ang speakers ay engaging at knowledgeable. Impressive talaga!",
  "Very memorable experience. Salamat sa opportunity na magattend!",
  "Ang best investment ko para sa professional development namin.",
];

// Neutral Comments - Mixed Tagalog, English, and Taglish (50 total)
...
// Negative Comments - Mixed Tagalog, English, and Taglish (50 total)
...
// Generate a random rating (1-5) with weighted distribution favoring higher ratings
...
const seedFormWith500Attendees = async () => {
  try {
    await connectDB();
    ...
    if (!creator) {
      console.log("User not found. Creating with your email...");
      creator = new User({
    ...
    // Delete existing form with same title
    await Form.deleteOne({ title: TARGET_FORM_TITLE });
    console.log(`Cleared previous form if existed.`);

    // Create form with questions
    const eventStartDate = new Date(2026, 0, 15); // Jan 15, 2026
    const eventEndDate = new Date(2026, 0, 16); // Jan 16, 2026

    const form = new Form({
      title: TARGET_FORM_TITLE,
      description:
        "Annual Evaluation Event with 500 attendees from Higher Education and Basic Education departments",
      type: "evaluation",
      status: "published",
    ...
    form.questions[0]._id = new mongoose.Types.ObjectId();
    form.questions[1]._id = new mongoose.Types.ObjectId();

    console.log(
      "Form structure created. Now generating 500 attendees and responses...\n",
    );

    // Generate 500 attendees - split between Higher Ed and Basic Ed
    const totalAttendees = 500;
    const higherEdCount = 250; // 250 from Higher Education
    const basicEdCount = 250; // 250 from Basic Education

    const attendeeList = [];
    const responses = [];

    const dateRangeStart = new Date(2026, 0, 1);
    const dateRangeEnd = new Date(2026, 1, 4);

    // Generate Higher Education attendees
    console.log("Generating Higher Education attendees (250)...");
    for (let i = 0; i < higherEdCount; i++) {
      ...
      // Generate response for this attendee
      const rating = generateRating();
      const comment = getComment(rating);
      const submittedDate = generateRandomDate(dateRangeStart, dateRangeEnd);
      ...
      if ((i + 1) % 50 === 0) {
        console.log(`  ✓ Generated ${i + 1} Higher Education attendees`);
      }
    }

    // Generate Basic Education attendees
    console.log("\nGenerating Basic Education attendees (250)...");
    for (let i = 0; i < basicEdCount; i++) {
      const firstName = firstNames[(higherEdCount + i) % firstNames.length];
      const lastName = lastNames[(higherEdCount + i) % lastNames.length];
      ...
      if ((i + 1) % 50 === 0) {
        console.log(`  ✓ Generated ${i + 1} Basic Education attendees`);
      }
    }

    // Assign attendees and responses to form
    ...
    console.log(
      `  Higher Education: ${higherEdCount} (${((higherEdCount / totalAttendees) * 100).toFixed(1)}%)`,
    );
    console.log(
      `  Basic Education: ${basicEdCount} (${((basicEdCount / totalAttendees) * 100).toFixed(1)}%)`,
    );
    console.log(`  Total Responses: ${responses.length}`);
    console.log(`  Quantitative Ratings (Scale 1-5): 500 responses`);
    console.log(`  Positive Comments: 50 (mixed Tagalog/English/Taglish)`);
    console.log(`  Neutral Comments: 50 (mixed Tagalog/English/Taglish)`);
    console.log(`  Negative Comments: 50 (mixed Tagalog/English/Taglish)`);
    console.log(
      `  Date Range: ${dateRangeStart.toLocaleDateString()} to ${dateRangeEnd.toLocaleDateString()}`,
    );
    console.log(`  Form Title: "${TARGET_FORM_TITLE}"`);
    console.log(`\n✨ All data has been seeded to the database!`);
