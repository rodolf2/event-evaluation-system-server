...
router.post('/upload', requireRole(['psas', 'club-officer']), formsController.uploadForm);

// POST /api/forms/extract-by-url - Extract data from Google Forms URL (all roles can extract for analysis)
router.post('/extract-by-url', requireRole(['psas', 'club-officer', 'school-admin', 'mis']), formsController.extractFormByUrl);

// POST /api/forms/extract-by-file - Extract questions from a file (all roles can extract for analysis)
router.post('/extract-by-file', requireRole(['psas', 'club-officer', 'school-admin', 'mis']), formsController.extractFormByFile);

// POST /api/forms/upload-by-url - Upload a Google Forms URL and create a form from it (psas, club-officer can create)
router.post('/upload-by-url', requireRole(['psas', 'club-officer']), formsController.uploadFormByUrl);

// PATCH /api/forms/:id/publish - Publish a form and generate shareable link (psas, club-officer can publish)
router.patch('/:id/publish', requireRole(['psas', 'club-officer']), formsController.publishForm);
